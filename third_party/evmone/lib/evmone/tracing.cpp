...
class HistogramTracer : public Tracer
{
    ...
    void on_instruction_start(uint32_t pc, const evmone::Stack& /*stack_ptr*/, int /*stack_height*/,
...
class InstructionTracer : public Tracer
{
    ...
    void output_stack(const evmone::Stack& stack, int stack_height)
    {
        m_out << R"(,"stack":[)";
        for (auto it = 0; it != stack_height; ++it)
        {
            if (it != 0)
                m_out << ',';
            m_out << R"("0x)" << to_string(stack.get(it), 16) << '"';
        }
        m_out << ']';
    }
    ...
    void on_instruction_start(uint32_t pc, const evmone::Stack& stack, int stack_height,
        const ExecutionState& state) noexcept override
    {
        const auto& ctx = m_contexts.top();
        ...
        output_stack(stack, stack_height);
