...
public:
    ...
    void notify_instruction_start(  // NOLINT(misc-no-recursion)
        uint32_t pc, const evmone::Stack& stack, int stack_height,
        const ExecutionState& state) noexcept
    {
        on_instruction_start(pc, stack, stack_height, state);
        if (m_next_tracer)
            m_next_tracer->notify_instruction_start(pc, stack, stack_height, state);
    }

private:
    virtual void on_execution_start(
        evmc_revision rev, const evmc_message& msg, bytes_view code) noexcept = 0;
    virtual void on_instruction_start(uint32_t pc, const evmone::Stack& stack_ptr, int stack_height,
