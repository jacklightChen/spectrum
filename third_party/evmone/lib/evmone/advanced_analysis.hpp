...
struct AdvancedExecutionState : evmone::ExecutionState {
  ...
  AdvancedExecutionState(const evmc_message& message, evmc_revision revision,
                         ...
                         evmc_host_context* host_ctx, bytes_view _code) noexcept
...
static_assert(sizeof(InstructionArgument) == sizeof(uint64_t),
              "Incorrect size of instruction_argument");

/// The pointer to function implementing an instruction execution.
using instruction_exec_fn = const Instruction* (*)(const Instruction*,
                                                   AdvancedExecutionState&);

/// The evmone intrinsic opcodes.
///
/// These intrinsic instructions may be injected to the code in the analysis
/// phase. They contain additional and required logic to be executed by the
/// interpreter.
enum intrinsic_opcodes {
  /// The BEGINBLOCK instruction.
  ///
  /// This instruction is defined as alias for JUMPDEST and replaces all
  /// JUMPDEST instructions. It is also injected at beginning of basic blocks
  /// not being the valid jump destination. It checks basic block execution
  /// requirements and terminates execution if they are not met.
...
struct Instruction {
  ...
  explicit constexpr Instruction(instruction_exec_fn f) noexcept
      : fn{f}, arg{} {}
};
...
struct AdvancedCodeAnalysis {
  ...
  /// This is value to which the next instruction pointer must be set in
  /// JUMP/JUMPI.
  std::vector<int32_t> jumpdest_targets;
};

inline int find_jumpdest(const AdvancedCodeAnalysis& analysis,
                         int offset) noexcept {
...
EVMC_EXPORT AdvancedCodeAnalysis analyze(evmc_revision rev,
