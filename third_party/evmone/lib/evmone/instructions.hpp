...
class StackTop {
 ...
 public:
  ...
  StackTop(Stack& _inner) noexcept : inner(_inner) { }

  /// The current number of items on the stack.
  [[nodiscard]] int size() const noexcept { return inner.size(); }

  /// Returns the reference to the top item.
  // NOLINTNEXTLINE(readability-make-member-function-const)
  [[nodiscard]] uint256& top() { return inner.top(); }

  /// Returns the reference to the stack item on given position from the stack
  /// top.
  // NOLINTNEXTLINE(readability-make-member-function-const)
  [[nodiscard]] uint256& get_mut(int index) { return inner.get_mut(index); }

  /// Returns the const reference to the stack item on given position from the
  /// stack top.
  [[nodiscard]] const uint256& get(int index) const { return inner.get(index); }

  /// Pushes an item on the stack. The stack limit is not checked.

  void push(const uint256& item) { return inner.push(item); }

  /// Returns an item popped from the top of the stack.
  uint256 pop() { return inner.pop(); }

  /// Empties the stack by resetting the top item pointer to the new provided
  /// stack space.
  void reset(uint256* space_bottom) { return inner.reset(space_bottom); }

  /// Switch to a new version by adding a new slice array
  std::tuple<uint32_t, uint32_t, uint256*> make_checkpoint() { return inner.make_checkpoint(); }

  /// Roll back to a given checkpoint
  void goto_checkpoint(std::tuple<uint32_t, uint32_t, uint256*> ckpt) { return inner.goto_checkpoint(ckpt); }

  std::int64_t rest_size() { return inner.rest_size(); }
};
...
// This function should not be inlined because this may affect other inlining
// decisions:
...
{
    // This implementation recomputes memory.size(). This value is already known
    // to the caller and can be passed as a parameter, but this make no
...
inline bool check_memory(ExecutionState& state, const uint256& offset, uint64_t size) noexcept
{
    ...
    // There is "branchless" variant of this using | instead of ||, but
    // benchmarks difference is within noise. This should be decided when moving
    // the implementation to intx.
...
inline bool check_memory(ExecutionState& state, const uint256& offset, const uint256& size) noexcept
{
    ...
    // However, compilers do decent although not perfect job unifying common
    // instructions.
    // TODO: This should be done in intx.
...
{
    auto top_item = stack.pop();
    stack.get_mut(0) += top_item;
}
...
{
    stack.get_mut(1) = stack.get(0) - stack.get(1);
}

inline void div(StackTop stack) noexcept
{
    auto& v = stack.get_mut(1);
    v = v != 0 ? stack.get(0) / v : 0;
}

inline void sdiv(StackTop stack) noexcept
{
    auto& v = stack.get_mut(1);
    v = v != 0 ? intx::sdivrem(stack.get(0), v).quot : 0;
}
...
{
    auto& v = stack.get_mut(1);
    v = v != 0 ? intx::sdivrem(stack.get(0), v).rem : 0;
}
...
inline void addmod(StackTop stack) noexcept
{
    ...
    auto& m = stack.get_mut(0);
    m = m != 0 ? intx::addmod(x, y, m) : 0;
}

inline void mulmod(StackTop stack) noexcept
{
    const auto& x = stack.get(0);
    const auto& y = stack.get(1);
    auto& m = stack.get_mut(2);
...
inline void signextend(StackTop stack) noexcept
{
    ...
    if (ext < 31)  // For 31 we also don't need to do anything.
    {
        ...
        // Sign-extend the "sign" byte and move it to the right position. Value
        // bits are zeros.
        ...
        // Produce bits (all zeros or ones) for extended words. This is done by
        // SAR of the sign-extended byte. Shift by any value 7-63 would work.
...
{
    const auto& x = stack.pop();
    stack.get_mut(0) = x < stack.get(0);
}

inline void gt(StackTop stack) noexcept
{
    const auto& x = stack.pop();
    stack.get_mut(0) = stack.get(0) < x;  // Arguments are swapped and < is used.
}

inline void slt(StackTop stack) noexcept
{
    const auto& x = stack.pop();
    stack.get_mut(0) = slt(x, stack.get(0));
}

inline void sgt(StackTop stack) noexcept
{
    const auto& x = stack.pop();
    stack.get_mut(0) = slt(stack.get(0), x);  // Arguments are swapped and SLT is used.
}
...
inline evmc_status_code sload(StackTop stack, ExecutionState& state) noexcept
{
    ...
    {
        // The warm storage access cost is already applied (from the cost
        // table). Here we need to apply additional cold storage access cost.
...
inline code_iterator jump_impl(ExecutionState& state, const uint256& dst) noexcept
{
    ...
    {
        if (dst >= jumpdest_map.size()) {
            LOG(INFO) << "CASE 1 BAD JUMPDEST ::: SIZE " << jumpdest_map.size() << std::endl;
        } else {
            LOG(INFO) << "CASE 2 BAD JUMPDEST ::: SIZE " << jumpdest_map.size() << std::endl;
        }
        // LOG(INFO) << dst[0] << ":" << dst[1] << ":" << dst[2] << ":" << dst[3] << std::endl;
...
///
/// It assumes that at lest 32 bytes of data are available so code padding is
/// required.
template <size_t Len>
...
/// @tparam N  The number as in the instruction definition, e.g. SWAP3 is
/// swap<3>.
template <int N>
inline void swap(StackTop stack) noexcept
{
    static_assert(N >= 1 && N <= 16);
    std::swap(stack.top(), stack.get_mut(N));
}
...
{
    const auto& offset = stack.get(0);
    const auto& size = stack.get(1);
...
inline StopToken selfdestruct(StackTop stack, ExecutionState& state) noexcept
{
    ...
    const auto beneficiary = intx::be::trunc<evmc::address>(stack.get(0));
