...
evmc::bytes32 IntraBlockState::get_storage(const evmc::address& address, const evmc::bytes32& key,
    ...
    // std::cout<<"enter get storage"<<std::endl;
    // std::cout<<"HZC get key: "<<to_hex(key)<<std::endl;
    auto* obj{get_object(address)};
    if (!obj || !obj->current) {
        return {};
    }
...
evmc::bytes32 IntraBlockState::get_storage_with_local(const evmc::address& address, const evmc::bytes32& key, bool original, const Transaction* txn) noexcept {
    // std::cout<<"enter get storage"<<std::endl;
    // std::cout<<"HZC get key: "<<to_hex(key)<<std::endl;
    auto* obj{get_object(address)};
    if (!obj || !obj->current) {
    ...
    // if (!original) {
    //     auto it{txn->current_state[address].find(key)};
    //      if (it != txn->current_state[address].end()) {
    //         return it->second;
    //     }
    // }
...
void IntraBlockState::set_storage(const evmc::address& address, const evmc::bytes32& key,
    ...
    // std::cout<<"enter set storage"<<std::endl;
    ...
    // std::cout<<"HZC set key: "<<to_hex(key)<<std::endl;
    // std::cout<<"HZC set value: "<<to_hex(value)<<std::endl;
...
void IntraBlockState::add_log(const Log& log) noexcept { 
  // HZC delete
  logs_.push_back(log); 
}
