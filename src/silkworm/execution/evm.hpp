...
class EvmTracer {
  public:
    ...
    virtual void on_execution_start(evmc_revision rev, const evmc_message &msg,
                                    evmone::bytes_view code) noexcept = 0;

    virtual void
    on_instruction_start(uint32_t pc, const evmone::Stack& stack,
                         int stack_height, const evmone::ExecutionState &state,
                         const IntraBlockState &intra_block_state) noexcept = 0;

    virtual void
    on_execution_end(const evmc_result &result,
                     const IntraBlockState &intra_block_state) noexcept = 0;

    virtual void
    on_precompiled_run(const evmc_result &result, int64_t gas,
                       const IntraBlockState &intra_block_state) noexcept = 0;

    virtual void
    on_reward_granted(const CallResult &result,
                      const IntraBlockState &intra_block_state) noexcept = 0;
};

using EvmoneExecutionState = evmone::ExecutionState;
...
class EVM {
  public:
    ...
    const std::vector<std::reference_wrapper<EvmTracer>> &
    tracers() const noexcept {
        return tracers_;
    };
    ...
    // Point to a cache instance in order to enable execution with evmone
    // advanced rather than baseline interpreter
    ...
    evmc::address beneficiary; // block.header.beneficiary by default; may be
                               // overridden for Clique
    bool is_twice{false};
    gsl::owner<EvmoneExecutionState*> execution_state{nullptr};
  ...
  private:
    ...
    evmc_result execute(const evmc_message &message, ByteView code,
                        const evmc::bytes32 *code_hash) noexcept;

    evmc_result execute_with_baseline_interpreter(
        evmc_revision rev, const evmc_message &message, ByteView code,
        const evmc::bytes32 *code_hash) noexcept;

    evmc_result execute_with_advanced_interpreter(
        evmc_revision rev, const evmc_message &message, ByteView code,
        const evmc::bytes32 &code_hash) noexcept;
...
class EvmHost : public evmc::Host {
  public:
    ...
    evmc_access_status
    access_account(const evmc::address &address) noexcept override;

    evmc_access_status
    access_storage(const evmc::address &address,
    ...
    evmc_storage_status
    set_storage(const evmc::address &address, const evmc::bytes32 &key,
                const evmc::bytes32 &value) noexcept override;

    evmc::uint256be
    get_balance(const evmc::address &address) const noexcept override;

    size_t get_code_size(const evmc::address &address) const noexcept override;

    evmc::bytes32
    get_code_hash(const evmc::address &address) const noexcept override;

    size_t copy_code(const evmc::address &address, size_t code_offset,
                     uint8_t *buffer_data,
    ...
    void emit_log(const evmc::address &address, const uint8_t *data,
