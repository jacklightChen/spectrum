...
struct AccessListEntry {
  ...
  friend bool operator==(const AccessListEntry &,
                         const AccessListEntry &) = default;
};
...
struct Transaction {
  ...
  std::optional<evmc::address> from{
      std::nullopt};  // sender recovered from the signature

  [[nodiscard]] intx::uint256 v() const;  // EIP-155

  //! \brief Returns false if v is not acceptable (v != 27 && v != 28 && v <
  //! 35, see EIP-155)
  ...
  [[nodiscard]] intx::uint256 priority_fee_per_gas(
      const intx::uint256 &base_fee_per_gas) const;  // EIP-1559
  [[nodiscard]] intx::uint256 effective_gas_price(
      const intx::uint256 &base_fee_per_gas) const;  // EIP-1559

  // HZC add r/w set
  // mutable FlatHashMap<evmc::address, FlatHashMap<evmc::bytes32,
  // evmc::bytes32>> current_state; mutable FlatHashMap<evmc::address,
  // FlatHashMap<evmc::bytes32, evmc::bytes32>> read_set; mutable
  // FlatHashMap<evmc::address, FlatHashMap<evmc::bytes32, evmc::bytes32>>
  // write_set;

  dcc::ExecType wrapped_exec_type;
  void *wrapped_txn;
...
// According to EIP-2718, serialized transactions are prepended with 1 byte
// containing the type (0x02 for EIP-1559 transactions); the same goes for
// receipts. This is true for signing and transaction root calculation. However,
// in block body RLP serialized EIP-2718 transactions are additionally wrapped
// into an RLP byte array (=string). (Refer to the geth implementation; EIP-2718
// is mute on block RLP.)
...
enum class Eip2718Wrapping {
  kNone,    // Serialized typed transactions must start with its type byte, e.g.
            // 0x02
  kString,  // Serialized typed transactions must be additionally wrapped into
            // an RLP string (=byte array)
  kBoth,    // Both options above are accepted
};

DecodingResult decode_transaction(
    ByteView &from, Transaction &to,
