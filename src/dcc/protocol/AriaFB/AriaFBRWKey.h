...
class AriaFBRWKey {
  public:
    ...
    uint32_t get_local_index_read_bit() const {
        return (bitvec >> LOCAL_INDEX_READ_BIT_OFFSET) &
               LOCAL_INDEX_READ_BIT_MASK;
    }
    ...
    void set_execution_processed_bit() {
        ...
        bitvec |= EXECUTION_PROCESSED_BIT_MASK
                  << EXECUTION_PROCESSED_BIT_OFFSET;
    }

    void clear_execution_processed_bit() {
        bitvec &=
            ~(EXECUTION_PROCESSED_BIT_MASK << EXECUTION_PROCESSED_BIT_OFFSET);
    }
  ...
  private:
     ...
     * prepare processed bit is set when process_request has processed this key
     * in prepare phase execution processed bit is set when process_request has
