...
SparklePartialTransaction::SparklePartialTransaction(Transaction&& inner, size_t id):
...
bool SparklePartialTransaction::HasWAR() {
...
void SparklePartialTransaction::SetWAR(const K& key, size_t cause_id) {
    auto guard = Guard{rerun_keys_mu};
...
SparklePartialTable::SparklePartialTable(size_t partitions):
...
void SparklePartialTable::Get(T* tx, const K& k, evmc::bytes32& v, size_t& version) {
    Table::Put(k, [&](V& _v) {
...
void SparklePartialTable::Put(T* tx, const K& k, const evmc::bytes32& v) {
    ...
    Table::Put(k, [&](V& _v) {
        ...
        // insert an entry
        _v.entries.insert(rit.base(), SparklePartialEntry {
            .value   = v,
            .version = tx->id,
...
void SparklePartialTable::RegretGet(T* tx, const K& k, size_t version) {
...
void SparklePartialTable::RegretPut(T* tx, const K& k) {
...
void SparklePartialTable::ClearGet(T* tx, const K& k, size_t version) {
    DLOG(INFO) << "remove read record " << tx->id << "(" << tx << ")" << " from " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
        auto vit = _v.entries.begin();
        auto end = _v.entries.end();
...
void SparklePartialTable::ClearPut(T* tx, const K& k) {
...
/// @param k the key of written entry
/// @return true if lock succeeds
bool SparklePartialTable::Unlock(T* tx, const K& k) {
...
SparklePartial::SparklePartial(Workload& workload, Statistics& statistics, size_t num_executors, size_t table_partitions, EVMType evm_type):
    workload{workload},
...
{
    LOG(INFO) << fmt::format("SparklePartial(num_executors={}, table_partitions={}, evm_type={})", num_executors, table_partitions, evm_type);
    workload.SetEVMType(evm_type);
}

/// @brief start spectrum protocol
/// @param num_executors the number of threads to start
void SparklePartial::Start() {
    stop_flag.store(false);
    for (size_t i = 0; i != num_executors; ++i) {
        executors.push_back(std::thread([this]{
            std::make_unique<SparklePartialExecutor>(*this)->Run();
        }));
...
SparklePartialExecutor::SparklePartialExecutor(SparklePartial& spectrum):
...
void SparklePartialExecutor::Generate() {
...
void SparklePartialExecutor::ReExecute() {
...
void SparklePartialExecutor::Finalize() {
...
void SparklePartialExecutor::Run() {
