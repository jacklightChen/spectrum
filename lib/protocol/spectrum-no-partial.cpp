...
/*
    This is a implementation of "SpectrumNoPartial: Speedy and Strictly-Deterministic Smart Contract Transactions for Blockchain Ledgers" (Zhihao Chen, Tianji Yang, Yixiao Zheng, Zhao Zhang, Cheqing Jin and Aoying Zhou). 
 */
...
#define V SpectrumNoPartialVersionList
#define T SpectrumNoPartialTransaction

/// @brief wrap a base transaction into a spectrum transaction
/// @param inner the base transaction
/// @param id transaction id
SpectrumNoPartialTransaction::SpectrumNoPartialTransaction(Transaction&& inner, size_t id):
...
bool SpectrumNoPartialTransaction::HasWAR() {
...
void SpectrumNoPartialTransaction::SetWAR(const K& key, size_t cause_id) {
    auto guard = Guard{rerun_keys_mu};
...
SpectrumNoPartialTable::SpectrumNoPartialTable(size_t partitions):
...
void SpectrumNoPartialTable::Get(T* tx, const K& k, evmc::bytes32& v, size_t& version) {
    Table::Put(k, [&](V& _v) {
...
void SpectrumNoPartialTable::Put(T* tx, const K& k, const evmc::bytes32& v) {
    ...
    Table::Put(k, [&](V& _v) {
        ...
        // insert an entry
        _v.entries.insert(rit.base(), SpectrumNoPartialEntry {
            .value   = v,
            .version = tx->id,
...
void SpectrumNoPartialTable::RegretGet(T* tx, const K& k, size_t version) {
...
void SpectrumNoPartialTable::RegretPut(T* tx, const K& k) {
...
void SpectrumNoPartialTable::ClearGet(T* tx, const K& k, size_t version) {
    DLOG(INFO) << "remove read record " << tx->id << "(" << tx << ")" << " from " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
        auto vit = _v.entries.begin();
        auto end = _v.entries.end();
...
void SpectrumNoPartialTable::ClearPut(T* tx, const K& k) {
...
SpectrumNoPartial::SpectrumNoPartial(Workload& workload, Statistics& statistics, size_t num_executors, size_t table_partitions, EVMType evm_type):
    workload{workload},
...
{
    LOG(INFO) << fmt::format("SpectrumNoPartial(num_executors={}, table_partitions={}, evm_type={})", num_executors, table_partitions, evm_type);
    workload.SetEVMType(evm_type);
}

/// @brief start spectrum protocol
/// @param num_executors the number of threads to start
void SpectrumNoPartial::Start() {
    stop_flag.store(false);
    for (size_t i = 0; i != num_executors; ++i) {
        executors.push_back(std::thread([this]{
            std::make_unique<SpectrumNoPartialExecutor>(*this)->Run();
        }));
...
SpectrumNoPartialExecutor::SpectrumNoPartialExecutor(SpectrumNoPartial& spectrum):
...
void SpectrumNoPartialExecutor::Generate() {
...
void SpectrumNoPartialExecutor::ReExecute() {
...
void SpectrumNoPartialExecutor::Finalize() {
...
void SpectrumNoPartialExecutor::Run() {
