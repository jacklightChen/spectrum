...
struct SpectrumNoPartialPutTuple {
    K               key;
    evmc::bytes32   value;
    bool            is_committed;
};

struct SpectrumNoPartialGetTuple {
    K               key;
    evmc::bytes32   value;
    size_t          version;
...
struct SpectrumNoPartialTransaction: public Transaction {
    size_t      id;
    ...
    std::vector<SpectrumNoPartialGetTuple>       tuples_get{};
    std::vector<SpectrumNoPartialPutTuple>       tuples_put{};
    SpectrumNoPartialTransaction(Transaction&& inner, size_t id);
    bool HasWAR();
    void SetWAR(const K& key, size_t cause_id);
};

struct SpectrumNoPartialEntry {
    evmc::bytes32   value;
    size_t          version;
...
struct SpectrumNoPartialVersionList {
    T*          tx = nullptr;
    std::list<SpectrumNoPartialEntry> entries;
    // readers that read default value
    std::unordered_set<T*>  readers_default;
};

struct SpectrumNoPartialTable: private Table<K, V, KeyHasher> {

    SpectrumNoPartialTable(size_t partitions);
...
using SpectrumNoPartialQueue = LockPriorityQueue<T>;
class SpectrumNoPartialExecutor;

class SpectrumNoPartial: public Protocol {

    private:
    size_t              num_executors;
    Workload&           workload;
    SpectrumNoPartialTable       table;
    ...
    friend class SpectrumNoPartialExecutor;

    public:
    SpectrumNoPartial(Workload& workload, Statistics& statistics, size_t num_executors, size_t table_partitions, EVMType evm_type);
    void Start() override;
    void Stop() override;
...
class SpectrumNoPartialExecutor {
    ...
    SpectrumNoPartialQueue           queue;
    std::unique_ptr<T>      tx{nullptr};
    std::barrier<std::function<void()>>&           stop_latch;

    public:
    SpectrumNoPartialExecutor(SpectrumNoPartial& spectrum);
    void Finalize();
    void Generate();
    void ReExecute();
    void Run();
