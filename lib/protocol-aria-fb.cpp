...
/*
    This is a implementation of "Aria: A Fast and Practical Deterministic OLTP
   Database" (Yi Lu, Xiangyao Yu, Lei Cao, Samuel Madden). In this
   implementation we adopt the fallback strategy and reordering strategy
...
Aria::Aria(Workload& workload, Statistics& statistics, size_t n_threads,
           size_t table_partitions, size_t batch_size, bool enable_reordering)
    : workload{workload},
  ...
  LOG(INFO) << fmt::format("Aria({}, {}, {}, {})", n_threads, table_partitions,
                           batch_size, enable_reordering)
            << std::endl;
}
...
void Aria::ParallelEach(std::function<void(T*)> map,
                        std::vector<std::unique_ptr<T>>& batch) {
  pool.submit_sequence(size_t{0}, batch.size(),
                       [&](size_t i) {
                         if (batch[i] == nullptr) {
...
#define LATENCY \
  duration_cast<microseconds>(steady_clock::now() - tx->start_time).count()
  pool.detach_task([&]() {
...
AriaTransaction::AriaTransaction(Transaction&& inner, size_t id,
                                 size_t batch_id)
...
void AriaTable::ReserveGet(T* tx, const K& k) {
  Table::Put(k, [&](auto& entry) {
    ...
    if (entry.reserved_get_tx == nullptr ||
        entry.reserved_get_tx->id > tx->id) {
      entry.reserved_get_tx = tx;
      DLOG(INFO) << tx->batch_id << ":" << tx->id << " reserve get ok"
                 << std::endl;
    }
  });
}
...
void AriaTable::ReservePut(T* tx, const K& k) {
  Table::Put(k, [&](auto& entry) {
    ...
    if (entry.reserved_put_tx == nullptr ||
        entry.reserved_put_tx->id > tx->id) {
      entry.reserved_put_tx = tx;
      DLOG(INFO) << tx->batch_id << ":" << tx->id << " reserve put ok"
                 << std::endl;
    }
  });
}
...
AriaLockTable::AriaLockTable(size_t partitions) : Table::Table(partitions) {}
...
void AriaExecutor::Execute(T* tx, AriaTable& table) {
  ...
  tx->UpdateGetStorageHandler(
      [tx, &table](const evmc::address& addr, const evmc::bytes32& key) {
        ...
        table.Get(tup, [&](auto& entry) { value = entry.value; });
        tx->local_get[tup] = value;
        return value;
      });
  // write locally to local storage
  tx->UpdateSetStorageHandler([tx, &table](const evmc::address& addr,
...
void AriaExecutor::Verify(T* tx, AriaTable& table, bool enable_reordering) {
  ...
  // therefore, we have to pick some transactions and arange them into a
  // sequence. this algorithm implicitly does it for us.
...
void AriaExecutor::Commit(T* tx, AriaTable& table) {
  for (auto& tup : tx->local_put) {
    table.Put(std::get<0>(tup),
              [&](auto& entry) { entry.value = std::get<1>(tup); });
  }
}
...
void AriaExecutor::PrepareLockTable(T* tx, AriaLockTable& table) {
  for (auto& tup : tx->local_get) {
    table.Put(std::get<0>(tup),
              [&](auto& entry) { entry.deps_get.push_back(tx); });
  }
  for (auto& tup : tx->local_put) {
    table.Put(std::get<0>(tup),
              [&](auto& entry) { entry.deps_put.push_back(tx); });
  }
}
...
void AriaExecutor::Fallback(T* tx, AriaTable& table,
  ...
  tx->UpdateSetStorageHandler([&](const evmc::address& addr,
                                  ...
                                  const evmc::bytes32& value) {
    auto tup = std::make_tuple(addr, key);
    table.Put(tup, [&](auto& entry) { entry.value = value; });
    return evmc_storage_status::EVMC_STORAGE_MODIFIED;
  });
  // get the latest dependency and wait on it
  T* should_wait = nullptr;
#define COND \
  (_tx->id < tx->id && (should_wait == nullptr || _tx->id > should_wait->id))
  for (auto& tup : tx->local_put) {
    lock_table.Get(std::get<0>(tup), [&](auto& entry) {
      for (auto _tx : entry.deps_get) {
        if (COND) {
          should_wait = _tx;
        }
      }
      for (auto _tx : entry.deps_put) {
        if (COND) {
          should_wait = _tx;
        }
      }
